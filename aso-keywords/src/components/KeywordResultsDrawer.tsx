      ...
      <aside ref={drawerRef} className="keyword-results-drawer" role="dialog" aria-modal="true" aria-labelledby="keyword-results-title">
        ...
        <div className="keyword-results-body">
          ...
          <section className="keyword-results-section">
            <header><div><h3>Топ‑5 приложений</h3><p>Последний сохранённый snapshot ASO Tracker; это не paid выдача.</p></div><span title="Источник и свежесть">Кэш · {formatFreshness(ranking?.lastUpdated)}</span></header>
        ...
        <footer className="keyword-results-footer"><span>Источник: ASO Tracker cached rankings · {formatFreshness(ranking?.lastUpdated)}</span><button type="button" onClick={onClose}>Готово</button></footer>
      </aside>
    </div>
  );
}
