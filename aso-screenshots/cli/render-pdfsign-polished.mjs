...
const staging='/tmp/pdfsign-polished-render-output';await fs.mkdir(staging,{recursive:true});
const browser=await chromium.launch({channel:'chrome',headless:true});
const selected=process.argv.slice(2);
const checks=selected.length?JSON.parse(await fs.readFile(`${root}/validation-polished.json`,'utf8')).filter(c=>!selected.some(n=>c.slot.startsWith(`pdfsign-real-${n}-`))):[];
for(const variant of manifest.filter(v=>!selected.length||selected.includes(v.id))){
 const state=JSON.parse(await fs.readFile(`${root}/states/${variant.id}.json`,'utf8'));delete state.ppo;state.screenshots.forEach((slot,i)=>{slot.sourceUrl=`/studio/pdfsign-polish/polished/${variant.id}-${i+1}.png`;slot.action=undefined;});
 ...
 for(const locale of ['en','ru','de-DE'])for(const slot of state.screenshots){
  ...
  const dest=`${staging}/${variant.id}-${slot.sampleIndex+1}-${locale}-polished.png`;
  await page.locator('[data-render-root]').screenshot({path:dest});
  const out=`/Users/qwar49/Developer/screenshots/PDFSign/polish-v2/variants/${variant.id}/polished/${locale}`;await fs.mkdir(out,{recursive:true});await fs.copyFile(dest,`${out}/${slot.sampleIndex+1}.png`);
 }
 await ctx.close();console.log('Rendered',variant.id,'EN/RU/DE');
}
await browser.close();for(const f of await fs.readdir(staging))await fs.copyFile(`${staging}/${f}`,`${root}/exports/${f}`);await fs.writeFile(`${root}/validation-polished.json`,JSON.stringify(checks,null,2));
let layers=[];
for(let r=0;r<manifest.length;r++)for(let c=0;c<3;c++){const img=await sharp(`${root}/exports/${manifest[r].id}-${c+1}-en-polished.png`).resize(264,574).toBuffer();layers.push({input:img,left:c*276,top:r*598});}
await sharp({create:{width:816,height:2990,channels:3,background:'#e9e9ed'}}).composite(layers).png().toFile(`${root}/overview-polished.png`);
