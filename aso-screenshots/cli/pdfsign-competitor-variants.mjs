...
const root=path.resolve('public/pdfsign-competitors');
const canonical='/Users/qwar49/Developer/screenshots/PDFSign';
const names=['acrobat','docusign','pdf-expert','signeasy','signnow'];
const W=1320,H=2868,base='/studio/pdfsign-competitors';
for(const d of ['artwork','source','states','references','exports'])await fs.mkdir(`${root}/${d}`,{recursive:true});
const source={};
for(const [key,num] of Object.entries({home:1,document:2,export:3,editor:4,library:5,draw:6})){
 const bytes=await fs.readFile(`${canonical}/source/iphone/${num}.png`);await fs.writeFile(`${root}/source/${key}.png`,bytes);
...
let uid=0;
function phone(src,x,y,w,rot=0,rim='#202024',outline=null){
 const h=w*2622/1206,p=13,clip=`clip${uid++}`;
 return `<g transform="translate(${x} ${y}) rotate(${rot} ${w/2} ${h/2})"><rect x="-9" y="-9" width="${w+18}" height="${h+18}" rx="${w*.105}" fill="${rim}" ${outline?`stroke="${outline}" stroke-width="8"`:''} filter="url(#shadow)"/><defs><clipPath id="${clip}"><rect x="${p}" y="${p}" width="${w-p*2}" height="${h-p*2}" rx="${w*.087}"/></clipPath></defs><image href="${src}" x="${p}" y="${p}" width="${w-p*2}" height="${h-p*2}" preserveAspectRatio="none" clip-path="url(#${clip})"/><rect x="${w*.375}" y="${p+16}" width="${w*.25}" height="${w*.069}" rx="${w*.0345}" fill="#060609"/></g>`;
}
function card(src,x,y,w,h,rot=0){const clip=`clip${uid++}`;return `<g transform="translate(${x} ${y}) rotate(${rot} ${w/2} ${h/2})"><rect width="${w}" height="${h}" rx="35" fill="white" filter="url(#shadow)"/><defs><clipPath id="${clip}"><rect x="10" y="10" width="${w-20}" height="${h-20}" rx="27"/></clipPath></defs><image href="${src}" x="10" y="10" width="${w-20}" height="${h-20}" preserveAspectRatio="xMidYMid slice" clip-path="url(#${clip})"/></g>`;}
...
for(const n of names){
 const cfg=config[n],meta=JSON.parse(await fs.readFile(`${canonical}/references/${n}/metadata.json`,'utf8'));
 const samples=[],slots=[];
 for(let i=0;i<3;i++){
  ...
  await fs.copyFile(`${canonical}/references/${n}/${i+1}.jpg`,`${root}/references/${n}-${i+1}.jpg`);
  ...
  samples.push(sample);
  slots.push({id:`pdfsign-${n}-${i+1}`,filename:`${n}-${i+1}.png`,device:'iphone',sourceUrl:sample.screenSrc,sourcePixelWidth:W,sourcePixelHeight:H,sourceLayout:'full-bleed',sourceScale:1,sourceOffsetX:0,sourceOffsetY:0,enhancedUrl:null,presetId:`pdfsign-${n}`,backgroundOverride:null,headline:{verb:sample.verb,descriptor:sample.descriptor,subhead:''},font:cfg.font,fontSize:48,titlePx:title,subPx:sub,textYFraction:y,headlineSafeBottomFraction:safe,textColorOverride:color,tiltDeg:0,tiltX:0,tiltY:0,deviceX:0,deviceY:0,deviceScale:1,textX:0,textY:0,breakout:false,pulseScreen:0,enhanceState:'idle',kind:'regular',sampleIndex:i});
 }
 const preset={id:`pdfsign-${n}`,name:cfg.name,kind:'real',description:cfg.desc,recommendedFor:'PDFSign / PDF signing',background:{type:'solid',css:cfg.bg},text:{font:cfg.font,weight:cfg.weight,color:cfg.color,align:cfg.align,uppercase:false},tiltDeg:0,breakout:'none',isGradient:false,decorationsHint:'Reference-derived layout. Keep headline zones empty in artwork; all marketing copy is a live localized layer.',samples};
 await fs.writeFile(`src/lib/presets/imported/pdfsign-${n}.json`,JSON.stringify(preset,null,2));
...
console.log('Created 5 presets, 15 textless compositions and editable EN/RU/DE states.');
