...
const staging='/tmp/pdfsign-render-output';await fs.mkdir(staging,{recursive:true});
const browser=await chromium.launch({channel:'chrome',headless:true});
const selected=process.argv.slice(2);
const checks=selected.length?JSON.parse(await fs.readFile(`${root}/validation.json`,'utf8')).filter(c=>!selected.some(n=>c.slot.startsWith(`pdfsign-${n}-`))):[];
for(const variant of manifest.filter(v=>!selected.length||selected.includes(v.id))){
 ...
 for(const locale of ['en','ru','de-DE'])for(const slot of state.screenshots){
  ...
  const out=`/Users/qwar49/Developer/screenshots/PDFSign/variants/${variant.id}/${locale}`;await fs.mkdir(out,{recursive:true});await fs.copyFile(dest,`${out}/${slot.sampleIndex+1}.png`);
 }
